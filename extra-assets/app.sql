...
--clean--
create table ciclo ( idciclo integer not null, ciclo integer, anio date, primary key (idciclo) );
create table rolrevision ( idrol integer not null, nombre varchar(10) not null, descripcion char(254), primary key (idrol) );
create table tipodocente ( idtipodocente integer not null, nombre varchar(10), primary key (idtipodocente) );
create table docente ( iddocente integer not null, idtipodocente integer not null, nombre varchar(10) not null, apellidos varchar(50) not null, cod_docente varchar(10), sexo varchar(1) not null, primary key (iddocente), foreign key (idtipodocente) references tipodocente (idtipodocente) on delete restrict );
create table local ( idlocal integer not null, codigoedificio varchar(5), nombrelocal varchar(30) not null, codigolocal varchar(5), capacidad integer, primary key (idlocal) );
create table tipotramite ( idtipotramite integer not null, nombrelocal varchar(30) not null, descripcion char(254), primary key (idtipotramite) );
create table tramite ( fechasolicitud date not null, idtramite integer not null, idlocal integer, idtipotramite integer not null, primary key (idtramite), foreign key (idlocal) references local (idlocal) on delete restrict, foreign key (idtipotramite) references tipotramite (idtipotramite) on delete restrict );
...
create table tipogrupo ( idtipogrupo integer not null, nombre varchar(10) not null, primary key (idtipogrupo) );
create table materia ( idmateria integer not null, nombre varchar(10) not null, codigo_materia varchar(7) not null, uvs integer not null, primary key (idmateria) );
create table materiaciclo ( idmatciclo integer not null, idmateria integer, idciclo integer, primary key (idmatciclo), foreign key (idciclo) references ciclo (idciclo) on delete restrict, foreign key (idmateria) references materia (idmateria) on delete restrict );
...
create table tipoevaluacion ( idtipoevaluacion integer not null, nombre varchar(10) not null, descripcion char(254), primary key (idtipoevaluacion) );
create table evaluacion ( idevaluacion integer not null, idgrupo integer, idtipoevaluacion integer, nombreevaluacion varchar(20) not null, fechasolicitud date not null, primary key (idevaluacion), foreign key (idgrupo) references grupomateriaciclo (idgrupo) on delete restrict, foreign key (idtipoevaluacion) references tipoevaluacion (idtipoevaluacion) on delete restrict );
...
create table diasnohabiles ( id_dias integer not null, idciclo integer, nombre varchar(10) not null, descripcion char(254), fechasolicitud date, primary key (id_dias), foreign key (idciclo) references ciclo (idciclo) on delete restrict );
...
create table parametro ( parametro varchar(10) not null, diashabiles integer );
...
create table testigo ( idtestigo integer not null, idtramite integer, idestudiante integer, justificacion char(254) not null, primary key (idtestigo), foreign key (idestudiante) references estudiante (idestudiante) on delete restrict, foreign key (idtramite) references tramite (idtramite) on delete restrict );