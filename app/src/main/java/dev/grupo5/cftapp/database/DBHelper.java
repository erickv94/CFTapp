...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        //triggers
        db.execSQL("CREATE TRIGGER if not exists actualizar_nota AFTER UPDATE ON detallerevision for each row BEGIN update estadoevaluacion set nota=new.resultado WHERE idestudiante = new.idestudiante; END");
        db.execSQL("CREATE TRIGGER if not exists eliminar_cascada_tipo_tramite AFTER DELETE ON tipotramite for each row BEGIN delete from tramite where idtipotramite=old.idtipotramite; END");
        db.execSQL("CREATE TRIGGER if not exists eliminar_cascada_tipo_grupo AFTER DELETE ON tipogrupo for each row BEGIN delete from grupomateriaciclo where idtipogrupo=old.idtipogrupo; END");
